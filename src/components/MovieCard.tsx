...
import { Star, Play, Plus, Tv } from 'lucide-react'
...
export const MovieCard: React.FC<MovieCardProps> = ({ movie, index = 0 }) => {
  ...
  // Anime links go to a watch page stub; non-anime go to details
  const mediaTypeForLink = isAnime ? 'tv' : rawMediaType

  const year = (movie.release_date || movie.first_air_date || '').slice(0, 4)
  const rating = movie.vote_average?.toFixed(1) || '0.0'

  // For anime: poster_path is already an absolute URL from Jikan
  // For TMDB: poster_path is a relative path like /abc.jpg
  ...
  return (
    <motion.div
      ...
      className="flex-shrink-0 w-[140px] sm:w-[160px] md:w-[180px] lg:w-[195px] group cursor-pointer"
    >
      <Link
